...
const fetchMints = async (poolId: Hex, chainId: SushiSwapV4ChainId) => {
...
const fetchBurns = async (poolId: Hex, chainId: SushiSwapV4ChainId) => {
...
const fetchSwaps = async (poolId: Hex, chainId: SushiSwapV4ChainId) => {
...
const fetchCollects = async (poolId: Hex, chainId: SushiSwapV4ChainId) => {
...
function useTransactionsV4(pool: V4Pool, opts: UseTransactionsV4Opts) {
  const { poolId } = pool
  return useQuery({
    queryKey: ['poolTransactionsV4', poolId, opts],
    queryFn: async () => {
...
const PoolTransactionsV4: FC<PoolTransactionsV4Props> = ({ pool }) => {
  const [type, setType] = useState<
    Parameters<typeof useTransactionsV4>['1']['type']
  ...
  const { data, isLoading } = useTransactionsV4(pool, opts)
